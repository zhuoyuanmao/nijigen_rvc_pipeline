...
> ⭐ **2026-08-07 全曲成品定稿**: `tokyo-summer-session_lovelive-cover_v18.wav` (227.9s / 62 乐句,
...
> (电平/动态/空间/母带)。本曲成品: `tokyo-summer-session_lovelive-cover_v18.wav` (227.9s)。
...
> **可调旋钮 (供后续口味微调)**: 人声/BGM 比 (**按原曲实测配平 −9dB**, §12.4a) · 齐唱 P (0.42) ·
> 混响 send (0.10) · 声像宽度 (±0.22) · 去相关量 (±11-13ms) · LUFS (−12)。

### 12.4a ⚠️ 人声/伴奏配平: 别用母带段落法, 要量总线

**踩过的坑**: v11 用"母带内人声段响度 − 纯伴奏段响度"来配平, 测得原曲 +3.1 / 本版 +3.2,
判定 MATCH。**这是假阳性** —— 直接量总线才发现人声实际比原曲响 **4.1 dB**。

原因是那个指标灵敏度极低: 混音是**功率相加**, 人声抬 4 dB 只把"有人声段"的总响度抬约
1.3 dB, 还要和编曲密度差异混在一起。**分辨率不足以定这个参数。**

**正确测法** (`tools/measure_balance.py`):
- **本方成品**: 混音链里人声总线与 BGM 总线在求和前是分开的, 直接量二者 RMS。
  LUFS 增益与限幅器都在公共路径上, 所以比值就是母带里的比值 —— 精确值, 无需分离。
- **原曲**: 需要**互补分解** (人声 + 伴奏 = 混音)。用单模型出两个 stem;
  注意 audio-separator 会对每个 stem **独立做峰值归一化**, 两条轨比例被打乱、相加不等于混音
  (本例残差只比混音低 12 dB)。用最小二乘解回两个缩放系数即可复原 (残差 → −84 dB)。
  **不要用 §3 那种 3 模型门控混合的人声** —— 它不互补, 比值会有偏差。
- **验证分离器有无偏差**: 拿本方成品当已知真值跑一遍同一分离器。本例它报 −1.0、
  真值也是 −1.0, 偏差 0.0 → 该测法在这种素材上可信。**没有这一步就不能声称原曲那边的数准。**
- 两边**对齐到同一 LUFS** 再比, 否则会把"商业母带更响"混进来 (原曲 −8.3 LUFS vs 本版 −12.4)。

**本曲结果**: 六声优版原曲 人声 −20.3 / 伴奏 −15.1 → **−5.2 dB**。
据此把人声目标从 −5.0 改为 **−9.0 dB**, v17 实测 −5.1 dB (差 0.1)。
> 顺带一提, 两版原曲的内部平衡本身就差 2.2 dB (二人版 −2.7 / 六声优版 −5.2) ——
> **配平前先确认对标的是哪一版**。

---

## 12.5 参照原唱的修音 (2026-08-07; 参照升级 2026-08-16)

用**原曲人声**当基准修翻唱音准。不重训模型, 纯后处理。逐句结果见 [MIX_TABLE.md](MIX_TABLE.md)
的「音准偏差 / 修音」两列; 实现 `tools/pitch_correct.py`。

> **参照的选择本身是个杠杆 (v16 起)**: 初版参照是二人对唱版, 任一时刻只有一个声部在唱 →
> 62 句里 20 句参照是"另一个声部", 只能标 n/a 跳过。换成**逐句选角与本 cast 对应的
> 六声优版**后, 可测句 42→**47**, 自动修音 16→**18 句** (几句此前无参照的台词/合唱
> 首次有了同声部基准)。**参照与 cast 的声部匹配度, 直接决定修音的覆盖率。**
> 换参照前必须过三道门: 伴奏漂移 (变速即变调, 本次 5 窗全 0ms)、调性一致
> (折叠差 +10 ct)、对齐到工程时间轴 (`tools/prep_ref_6seiyuu.py`)。

### 先测量, 别默认要修
本曲实测: **中位偏差 20 音分, 系统偏移 +2 音分** —— 翻唱本来就是准的
(RVC 忠实保留源演唱的 F0, 所以音准问题只可能来自演唱本身)。
本曲 **21/62 句**有修音 (18 句自动曲线 + 1 句放宽参数台词曲线 + 2 句耳测定值);
另有 1 句台词被耳测否决、1 句因是耳测搭档而冻结。
**盲目全曲修音 = 给 62 句都付代价去修一个基本不存在的问题。**
...
### ⚠️ 耳测锁定的是**关系**, 搭档句必须一起冻结

差点踩的坑 (v16 换参照时被复核拦下): 0:15 男声合 Uh 的耳测 **−60** 是在
"鸟♀ 原样 D#4"的前提下 A/B 出来的 —— 用户听的是**这对组合**, 定的是**音程** (回到大三度)。
新参照下自动流程测出鸟♀ 那句偏 +60、要往下修 60 —— 单看她自己完全合规,
但一修, 组合就从"大三度"又塌回"小三度", **用户拍板过的听感被静默拆掉**。

> **规则**: 每个耳测/A/B 决定入保护清单时, 必须**连同它的搭档句** (同时发声的另一声部 /
> 呼应句) 一起冻结 —— 保护的不是那一句, 是那**对关系**。搭档句将来若真要修,
> 只能带着已锁的半边**重新 A/B 整个组合**, 不能单边动。
> (实现: `pitch_correct.py` 的 `PROTECT` 表按音色列时间段, 自动跳过。)
...
> **为什么不能"只补动过的那条轨"** (2026-08-17 复核): 各音色的修音**互相独立**
> (每条轨只用自己的 raw 和参照), 单看这一层确实可以只重跑动过的那条。真正的阻碍在于
> **链路里有非幂等的一步** —— 台词句修正 `fix_two_lines.py` 是在**已修音的分轨上叠加**
> (而非像耳测/逐音那样从 raw 重建), 再跑一次就会把那句修两次。
> 所以规则成立, 但理由要记准: **不是"修音有跨轨依赖", 而是"链路里有就地叠加的步骤"**。
> 若将来要支持增量重建, 就得先把那一步改成"先从备份还原再施加"。

> **反过来这是个免费的验收**: 重跑后**修正值变化的句子应当恰好落在被重对轨的区段内**。
> 若某句没被碰过却变了值, 说明测量不稳定 (或 raw 被意外改动), 要查而不是照单接受。
> 本曲 18 句里变了 4 句, 全部落在改动区段内, 其余 14 句一个音分未动 → 通过。

### ⚠️ 参照不可用时之二: 等律半音格 (逐音修正)

曲内平行段自洽只在"这句有结构对位的另一句"时可用。若既没有可比参照、也没有平行段
(2:48.8 海♂「時を止め帰りたくないよね」: 原始偏差 +1870 音分, 八度折叠后仅 11% 的帧落在
60 音分内 —— 原曲此处是另一条旋律), 就退到**等律半音格**当基准, 即经典 autotune。

两条护栏, 缺一不可:

1. **只修偏差 ≤50 音分的音**。超过一半就无法判断"最近半音"是不是本意, 机械对齐会把音修错
   (本例 11 个音全在 ±43 音分内, 无歧义)。
2. **按音整体平移, 不压平轮廓**。每个音施加一个常数偏移、音间平滑过渡 →
   颤音/滑音/起音全部保留; 压平会得到机器音。

强度取 **85%** 而非 100%: 留一点自然浮动。本例 6/11 个音被修, 离半音格的中位偏差
27 → 17 音分。实现 `tools/fine_tune_grid.py`。

> **三级判据的优先序**: 参照同声部 > 曲内平行段/和声关系 > 等律格。
> 越往下越不依赖外部信息, 也越容易修错 —— 所以每降一级, 护栏就要更严。
