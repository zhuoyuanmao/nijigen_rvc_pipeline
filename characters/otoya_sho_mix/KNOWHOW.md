...
> ✅ **2026-07-26 成品定稿** = **baseline 底模 + 中值谱 ensemble** (整曲 159.9s)
> (`output/final/FINAL_otoya_v2.wav`), 用户 3 路 A/B 耳测选定 (见 §10.5)。
...
**这一轮 (2026-07-25) 全部在噪声内, 逐段选优没买到任何东西**, 故当时定"单 e140"。
但**这一轮没有把中值谱 ensemble 纳入对比** —— 见 §10.5, 后来加了中值谱后结论翻转。

> ⚠️ **本小节的"单 e140 定稿"已被 §10.5 取代**。`FINAL_v2_vocals.wav` (单 e140) 保留为历史候选;
> 现行成品是 `FINAL_otoya_v2.wav` (baseline 中值谱)。`_build_single_ckpt.py` 仍可用作简版。

### 10.5 成品定稿: baseline 中值谱 ensemble (用户耳测, 2026-07-26)

后续给 otoya 加了 TITAN A/B 臂 (`_ab_titan.py`), 顺带把 **baseline 底模也做了中值谱
ensemble** (e100-200 窗口逐 bin 幅度谱中值 + 中间 ckpt 相位)。生成 3 路 A/B 供耳测:

```
_AB_titan/0_CURRENT_baseline_e140.wav   # 单 ckpt e140 (§10.4b 旧定稿)
_AB_titan/1_baseline_median.wav         # ⭐ baseline 底模 + 中值谱 → 用户选定
_AB_titan/2_titan_median.wav            # TITAN 底模 + 中值谱 (音色略差, timbre ~41 贴噪声带)
```

- **用户定稿 = `1_baseline_median`** → 归集为 `output/final/FINAL_otoya_v2.wav` (整曲 159.9s)。
- **客观 vs 主观**: 中值谱 vs 单 ckpt 的谐波间平坦度在噪声内 (0.057 vs 0.054), 但**耳测
  中值那层平滑/去噪更干净**, 主观胜出。**耳朵是成品的最终裁判**, 客观差异是噪声级时听感说了算。
- **底模仍用 baseline (官方 f0G40k)**: TITAN 在这个男嗓上音色略差、无空气感缺陷可补, 用不上。

> **修订的核心教训 (对后续音色)**: 训练做对后, transpose/修复链/波形平均 ensemble 确实不需要;
> **但中值谱 ensemble 是例外** —— 它是零成本去噪 (谐波间和声/沙噪), **男女声都值得默认做**。
> otoya 这轮把它从"亮嗓专用"升级成"统一默认" (见 [METHODOLOGY §7.3](../../METHODOLOGY.md))。
> 男女声的唯一差异回到**底模** (男 f0G40k / 女 TITAN)。
