"""kotori v4.5 training-corpus re-prep (58 raw tracks, single voice).
...
Same chain as otoya_sho_mix (METHODOLOGY §3): per raw track
  ...
  -> anvuew dereverb -> trim + LUFS -20 -> data/v45_corpus/kotori_<idx>.wav
...
RAW_DIR = Path(os.environ.get(
    "RAW_DIR", str(Path(__file__).resolve().parents[2] / "raw"))) / "kotori_raw"
...
def main() -> int:
    ...
    print(f"kotori: {len(tracks)} raw tracks from {RAW_DIR}", flush=True)
